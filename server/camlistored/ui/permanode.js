 ...
 * @private
 */
camlistore.PermanodePage.prototype.reloadTags_ =
function(permanode, describeResult) {
...
function(pn, path, meta, thumbnails) {
  ...
  if (thumbnails) {
    ...
    goog.events.listen(del,
                       goog.events.EventType.CLICK,
                       this.deleteMemberFunc_(pn, path, a, li),
                       false, this
                      );
...
function(member, path, strikeEle, removeEle) {
  var delFunc = function(e) {
    strikeEle.innerHTML = "<del>" + strikeEle.innerHTML + "</del>";
    this.connection_.newDelAttributeClaim(
      getPermanodeParam(),
