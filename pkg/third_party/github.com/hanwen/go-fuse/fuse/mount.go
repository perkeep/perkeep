...
func mount(mountPoint string) (f *os.File, finalMountPoint string, err error) {
...
func getFuseConn(local *os.File) (f *os.File, err error) {
	var data [4]byte
	...
	if message.Type != 1 {
		err = errors.New(fmt.Sprintf("getFuseConn: recvmsg returned wrong control type: %d", message.Type))
		return
	}
	if oobn <= syscall.SizeofCmsghdr {
		err = errors.New(fmt.Sprintf("getFuseConn: too short control message. Length: %d", oobn))
		return
	}
	if fd < 0 {
		err = errors.New(fmt.Sprintf("getFuseConn: fd < 0: %d", fd))
		return
	}
