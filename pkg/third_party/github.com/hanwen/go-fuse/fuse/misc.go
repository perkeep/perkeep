...
func MakeTempDir() string {
	...
	if err != nil {
		panic("TempDir() failed: " + err.Error())
	}
	return nm
}

// Convert os.Error back to Errno based errors.
func OsErrorToFuseError(err error) Status {
	if err != nil {
		...
		if ok {
			return OsErrorToFuseError(asPathErr.Err)
		}

		asLinkErr, ok := err.(*os.LinkError)
		if ok {
			return OsErrorToFuseError(asLinkErr.Err)
		}
...
func CopyFileInfo(fi os.FileInfo, attr *Attr) {
	...
	attr.Mtime = uint64(fi.ModTime() / 1e9)
	attr.Mtimensec = uint32(fi.ModTime() % 1e9)
