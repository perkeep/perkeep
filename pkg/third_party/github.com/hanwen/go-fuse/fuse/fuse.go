...
func (me *MountState) Mount(mountPoint string) error {
...
func (me *MountState) Loop(threaded bool) {
	...
	if me.threaded {
		me.errorChannel = make(chan error, 100)
...
func (me *MountState) Error(err error) {
...
func (me *MountState) Write(req *fuseRequest) {
	...
	if err != nil {
		me.Error(errors.New(fmt.Sprintf("writer: Writev %v failed, err: %v. Opcode: %v",
...
func (me *MountState) readRequest(req *fuseRequest) error {
...
func (me *MountState) discardFuseRequest(req *fuseRequest) {
	...
	key = opname + "-write"
	me.operationLatencies[key] += (endNs.Sub(req.preWriteNs))
	me.operationCounts[key] += 1
...
func (me *MountState) loop() {
	...
	for {
		...
		if err != nil {
			...
			readErr := errors.New(fmt.Sprintf("Failed to read from fuse conn: %v", err))
			me.Error(readErr)
			break
		}
...
func (me *MountState) handle(req *fuseRequest) {
	req.dispatchNs = time.Now()
	req.arg = bytes.NewBuffer(req.inputBuf)
	err := binary.Read(req.arg, binary.LittleEndian, &req.inHeader)
	if err == io.EOF {
		err = errors.New(fmt.Sprintf("MountPoint, handle: can't read a header, in_data: %v", req.inputBuf))
	}
	if err != nil {
		me.Error(err)
		return
...
func (me *MountState) dispatch(req *fuseRequest) {
	...
	// case FUSE_INTERRUPT
	default:
		me.Error(errors.New(fmt.Sprintf("Unsupported OpCode: %d=%v", h.Opcode, operationName(h.Opcode))))
		req.status = ENOSYS
		return
	}
