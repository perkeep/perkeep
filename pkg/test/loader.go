...
func (ld *Loader) GetStorage(prefix string) (blobserver.Storage, error) {
	...
	sto := ld.genStorage(prefix)
	ld.sto[prefix] = sto
	return sto, nil
}

func (ld *Loader) genStorage(prefix string) blobserver.Storage {
	if strings.HasPrefix(prefix, "/good") {
		return &Fetcher{}
	}
	if strings.HasPrefix(prefix, "/fail") {
		return &Fetcher{ReceiveErr: errors.New("test.Loader intentional failure for /fail storage handler")}
	}
