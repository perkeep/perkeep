...
func (mgw *MongoWrapper) getConnection() (*mgo.Session, os.Error) {
...
func (mgw *MongoWrapper) getCollection() (*mgo.Collection, os.Error) {
...
func newMongoIndex(mgw *MongoWrapper) (*Index, os.Error) {
...
func newMongoIndexFromConfig(ld blobserver.Loader, config jsonconfig.Obj) (blobserver.Storage, os.Error) {
...
func (s mongoStrIterator) Close() os.Error {
...
func (mk *mongoKeys) Get(key string) (string, os.Error) {
	mk.mu.Lock()
	defer mk.mu.Unlock()
	res := bson.M{}
...
func (mk *mongoKeys) Set(key, value string) os.Error {
	mk.mu.Lock()
	defer mk.mu.Unlock()
...
func (mk *mongoKeys) Delete(key string) os.Error {
	mk.mu.Lock()
	defer mk.mu.Unlock()
	if key == "" {
...
func (mk *mongoKeys) CommitBatch(bm BatchMutation) os.Error {
	b, ok := bm.(*batch)
	if !ok {
