...
// simple wrapper to allow for generic file reader vars
type GenericFileReader interface {
	FileSchema() *Superset
}
...
// to read the blobref entries in the static-set of a blobref of type directory 
...
// creates a new directory reader and prepares to fetch the static-set entries
func NewDirReader(fetcher blobref.SeekFetcher, fileBlobRef *blobref.BlobRef) (*DirReader, os.Error) {
	if fileBlobRef == nil {
		return nil, os.NewError("schema/filereader: NewDirReader blobref was nil")
	}
	ss := new(Superset)
	rsc, _, err := fetcher.Fetch(fileBlobRef)
	if err != nil {
		return nil, fmt.Errorf("schema/filereader: fetching directory schema blob: %v", err)
	}
	if err = json.NewDecoder(rsc).Decode(ss); err != nil {
		return nil, fmt.Errorf("schema/filereader: decoding directory schema blob: %v", err)
	}
	if ss.Type != "directory" {
		return nil, fmt.Errorf("schema/filereader: expected \"directory\" schema blob, got %q", ss.Type)
	}
	dr, _ := ss.NewDirReader(fetcher)
	staticSetBlobref := blobref.Parse(dr.FileSchema().Entries)
	if staticSetBlobref == nil {
		return nil, fmt.Errorf("schema/filereader: Invalid (static-set) blobref\n")
	}
	rsc, _, err = fetcher.Fetch(staticSetBlobref)
	if err != nil {
		return nil, fmt.Errorf("schema/filereader: fetching static-set schema blob: %v", err)
	}
	ssss := new(Superset)
	if err = json.NewDecoder(rsc).Decode(ssss); err != nil {
		return nil, fmt.Errorf("schema/filereader: decoding static-set schema blob: %v", err)
	}
	if ssss.Type != "static-set" {
		return nil, fmt.Errorf("schema/filereader: expected \"static-set\" schema blob, got %q", ssss.Type)
	}
	dr.staticSet = ssss.Members
	dr.current = 0
	return dr, nil
}
...
func (dr *DirReader) FileSchema() *Superset {
	return dr.ss
}

// returns the whole of the static set members of that directory
func (dr *DirReader) StaticSet() []string {
	return dr.staticSet
}

// returns up to n entries of the directory.
// if n > 0 and n > remaining entries, entries contains the
// result but the error is os.EOF.
// if n <= 0, it returns all the remaining entries and error
// is nil, except if an error occurs.
func (dr *DirReader) Read(n int) (entries []*DirEntry, err os.Error) {
	ss := dr.StaticSet()
	up := dr.current + n
	if n <= 0 {
		up = len(ss)
	} else {
		if n > (len(ss) - dr.current) {
			err = os.EOF
			up = len(ss)
		}
	}
	var entryR GenericFileReader
	for _, v := range ss[dr.current:up] {
		entryBref := blobref.Parse(v)
		if entryBref == nil {
			log.Printf("schema/filereader: invalid (static-set member) blobref\n")
			continue
		}
		entryR, err = NewDirReader(dr.fetcher, entryBref)
		if err != nil {
			entryR, err = NewFileReader(dr.fetcher, entryBref)
			if err != nil {
				entries = nil
				err = fmt.Errorf("schema/filereader: can't fetch dir or file: %v\n", err)
				break
			}
		}
		entry := &DirEntry{
			Name:    entryR.FileSchema().FileName,
			Type:    entryR.FileSchema().Type,
			Blobref: v,
		}
