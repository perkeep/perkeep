...
func RenderMembers() {
	var err error
	theBic, err = newblobItemContainer(200)
	if err != nil {
...
func subjectBasePath() (string, error) {
	basePath := js.Global.Get("subjectBasePath").String()
	if basePath == "" {
...
func subject() (blob.Ref, error) {
	sbj := js.Global.Get("subject").String()
	if sbj == "" {
...
func pathPrefix() (string, error) {
	prefix := js.Global.Get("pathPrefix").String()
	if prefix == "" {
