...
func TestDecodePortraitHEIC(t *testing.T) {
	...
	for _, baseName := range sampleNames(t) {
		wantDim := dimensions{4284, 5712}
		switch {
		case strings.HasPrefix(baseName, "mr-coffee-"):
		...
		t.Run(baseName, func(t *testing.T) {
			...
			t.Run("image.DecodeConfig", func(t *testing.T) {
				...
				if got != wantDim && wantFormat == "jpeg" {
					t.Logf("expected mismatch from std image package: std decode = %v (not %v post-EXIF)", got, wantDim)
				}
			})
